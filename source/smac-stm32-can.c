...
static smacRetCode_t smac_can_receive(smacCan_t can, uint32_t channel, smacCanMessage* message,
                                      uint32_t timeout)
{
...
{
    return smac_can_receive(can, CAN_RX_FIFO0, message, timeout);
}
...
{
    return smac_can_receive(can, CAN_RX_FIFO1, message, timeout);
}
...
static smacRetCode_t smac_can_async_receive(smacCan_t can, uint32_t channel,
...
/// CAN instance.
/// @details This function is a convenience wrapper around @ref smac_can_async_receive,
/// specifically for channel 0.
smacRetCode_t smac_can_async_receive_channel0(smacCan_t can, smacCanMessage* message)
{
    return smac_can_async_receive(can, 0, message);
}
...
{
    return smac_can_async_receive(can, 1, message);
}
...
static void on_rx_fifo_msg_pending_callback(CAN_HandleTypeDef* hcan, uint32_t fifo)
{
    ...
    smacRetCode_t code =
        smac_can_receive(cache->device, fifo, (smacCanMessage*)cache->cache_data[fifo], 0);
